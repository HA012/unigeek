...
int CC1101Util::_tunedRssi(float mhz) {
  cc1101SetFrequency(mhz);             // antenna path + FREQ registers
...
int CC1101Util::rssiAt(float mhz) {
  ...
  // Fast sweep path: unlike _tunedRssi(), don't force a fresh VCO calibration for
  // every pixel. beginRssiSweep() calibrates once near the band's midpoint; here
  // we only retune and let the PLL/AGC relock.
