...
void UidLibraryScreen::details(){_state=DETAILS;_d0=baseName(_path);_d1="ISO14443A";_d2=uidHex(_uid,_uidLen);_details[0]={"File",_d0.c_str()};_details[1]={"Type",_d1.c_str()};_details[2]={"UID",_d2.c_str()};_details[3]={"[Press]","Actions"};setItems(_details,4);}
...
void UidFormScreen::onItemSelected(uint8_t i){if(_state==SOURCE_SELECT){_source=(Source)i;if(_source==RANDOM)randomize();if(_source==SAVED)_savedPath="";rebuild();return;}if(_state==LENGTH_SELECT){size_t next=i==0?4:(i==1?7:10);if(next!=_uidLen){_uidLen=next;if(_source==RANDOM)randomize();else if(_source==MANUAL){memset(_uid,0,sizeof(_uid));}}rebuild();return;}if(_state==FILE_SELECT){if(i>=_browser.count())return;auto e=_browser.entry(i);if(e.isDir){_pickDir=e.path;chooseFile();return;}size_t n=0;if(IdentityFile::loadNfcUid(e.path,_uid,sizeof(_uid),n)){_uidLen=n;_savedPath=e.path;rebuild();}else ShowStatusAction::show("Invalid UID file",1600);return;}uint8_t row=0;if(i==row++)return;if(i==row++){chooseSource();return;}if(_source==SAVED){if(i==row++){chooseFile();return;}}else{if(i==row++){chooseLength();return;}if(_source==MANUAL&&i==row++){manual();return;}}if(i==row)save();}
...
void IdFormScreen::onItemSelected(uint8_t i){if(_state==TYPE_SELECT){auto f=LFCodec::formatAt(i);if(f){LFCodec::create(f->protocol,_data);if(_source==RANDOM){if(f->capabilities&LFCodec::CanGenerateRandom)randomize();else _source=MANUAL;}}rebuild();return;}if(_state==SOURCE_SELECT){auto info=LFCodec::format(_data.protocol);bool canRandom=info&&(info->capabilities&LFCodec::CanGenerateRandom);_source=(Source)(canRandom?i:i+1);if(_source==RANDOM)randomize();if(_source==SAVED)_savedPath="";rebuild();return;}if(_state==FILE_SELECT){if(i>=_browser.count())return;auto e=_browser.entry(i);if(e.isDir){_pickDir=e.path;files();return;}LFCodec::DecodedData d;if(IdentityFile::loadLfId(e.path,d)){_data=d;_savedPath=e.path;rebuild();}else ShowStatusAction::show("Invalid ID file",1600);return;}uint8_t row=0;if(_source!=SAVED){if(i==row++){types();return;}}if(i==row++){sources();return;}if(_source==SAVED){if(i==row++){files();return;}if(_savedPath.length()&&i==row++)return;}else if(_source==MANUAL){size_t n=LFCodec::editableFieldCount(_data.protocol);if(i>=row&&i<row+n){editField(i-row);return;}row+=n;}if(i==row)save();}
