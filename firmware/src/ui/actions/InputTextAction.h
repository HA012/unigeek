...
public:
  ...
  enum Profile : uint8_t { PROFILE_STANDARD = 0, PROFILE_HID = 1, PROFILE_TERMINAL = 2 };
...
private:
  ...
  void _buildSets() {
    ...
    if (_mode == INPUT_HEX) {
      ...
      if (_page == PAGE_HID && _profile != PROFILE_STANDARD) {
        static constexpr const char* hidLabels[30] = {
          "CTRL", "SHIFT", "ALT", "META", "ESC", "TAB",
          "",     "LEFT",  "UP",  "DOWN", "RIGHT", "",
        ...
        const char* saveLabel = (_profile != PROFILE_STANDARD) ? "SEND" : "SAVE";
        Special saveAction = (_profile != PROFILE_STANDARD) ? SP_HID_SEND : SP_SAVE;
  ...
  void _hidSendBuffer() {
    ...
    // Keep the composed command available when the transport rejects the
    // write. Terminal profiles stay open on failure, allowing the user to
    // retry or edit instead of silently losing the command.
    if (_profile != PROFILE_TERMINAL || _hidKeyboard->lastWriteOk()) {
      _input = "";
      _pendingChar = "";
      _tapCount = 0;
      _lastTapTime = 0;
  ...
  void _handleSelect(bool shifted = false) {
    ...
    if (s.isSpecial) {
      ...
      switch (s.special) {
        ...
        case SP_DELETE:
          if (_profile != PROFILE_STANDARD && _hidModifiers != 0) {
        ...
        case SP_PAGE_ABC:
          if (_mode == INPUT_TEXT && _profile != PROFILE_STANDARD) {
            _page = PAGE_ABC;
            _symbolMode = false;
            _buildSets();
            _scrollPos = 30;
        ...
        case SP_HID_SEND:
          _hidSendBuffer();
          if (_profile == PROFILE_TERMINAL && _hidKeyboard && _hidKeyboard->lastWriteOk()) _done = true;
          break;
  ...
  void _drawGridCell(int idx) {
    ...
    if (!s.isSpecial && _mode == INPUT_TEXT && s.chars) {
      ...
      lbl = (_page == PAGE_ABC) ? "123" : ((_profile != PROFILE_STANDARD) ? "HID" : "ABC");
    } else {
      lbl = String(s.label);
    }
