...
void ChameleonHIDProxScreen::_buildResult() {
  _rowCount = 0;
  String hex;
  char h[3];
  for (uint8_t i = 0; i < _payloadLen; i++) {
    snprintf(h, sizeof(h), "%02X", _payload[i]);
    hex += h;
  }
...
void ChameleonHIDProxScreen::_doLoadSlot() {
  auto& c = ChameleonClient::get();
...
void ChameleonHIDProxScreen::_saveToFile() {
  if (!Uni.Storage || !Uni.Storage->isAvailable() || !_payloadLen) {
    ShowStatusAction::show("Failed", 1200);
    render();
    return;
  }

  String type = "HID Prox";
  ...
  char h[3];
  for (uint8_t i = 0; i < (uint8_t)(_payloadLen); ++i) {
    snprintf(h, sizeof(h), "%02X", _payload[i]);
    suggested += h;
  }
  ...
  bool ok = false;
  if (f) {
    ok = f.write(_payload, _payloadLen) == (size_t)(_payloadLen);
    f.close();
  }
...
void ChameleonHIDProxScreen::_showActions() {
  static const InputSelectAction::Option opts[] = {
    {"Load to Slot", "slot"}, {"Save to File", "save"}, {"Write to Tag (T5577)", "t5577"},
  };
  const char* r = InputSelectAction::popup("HID Prox Actions", opts, 3, nullptr);
